...
public class ViewFoodComplaint extends AppCompatActivity {
    ...
    private void showComplaintDialog(Model complaint) {
        AlertDialog.Builder builder = new AlertDialog.Builder(ViewFoodComplaint.this);
    ...
    private void updateComplaintStatus(long timestamp, String newStatus) {
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if (complaint != null && complaint.getTimestamp() == timestamp) {
                        ...
                        snapshot.getRef().child("status").setValue(newStatus)
                                .addOnSuccessListener(aVoid -> {
                                    Toast.makeText(ViewFoodComplaint.this, "Complaint status updated to " + newStatus, Toast.LENGTH_SHORT).show();
                                })
                                .addOnFailureListener(e -> {
                                    Toast.makeText(ViewFoodComplaint.this, "Failed to update status", Toast.LENGTH_SHORT).show();
                                });
                        complaintFound = true;
                        break;
                    }
                ...
                if (!complaintFound) {
                    Toast.makeText(ViewFoodComplaint.this, "Complaint not found", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(ViewFoodComplaint.this, "Failed to update complaint", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
